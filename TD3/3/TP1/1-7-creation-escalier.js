...
function createStairs() {
	...
	for ( var nb = 1 ; nb < 6; nb++ ){
		...
		stepMesh = new THREE.Mesh( stepVertical, stepMaterialVertical );
		stepMesh.position.x = 0;
		stepMesh.position.y = (nb+1/2)*verticalStepHeight + nb*stepThickness;
		stepMesh.position.z = nb*horizontalStepDepth - 2*nb*stepHalfThickness;
		scene.add( stepMesh );

		stepMesh = new THREE.Mesh( stepHorizontal, stepMaterialHorizontal );
		stepMesh.position.x = 0;
		stepMesh.position.y = (nb+1/2)*stepThickness + (nb+1)*verticalStepHeight;
		stepMesh.position.z = (nb+1/2)*horizontalStepDepth - (nb*2+1)*stepHalfThickness;
		scene.add( stepMesh );
